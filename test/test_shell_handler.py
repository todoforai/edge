...
async def test_successful_run_no_output():
    ...
    assert any("Successful run" in msg for msg in client.messages), f"Expected 'Successful run' message, got: {client.messages}"
...
async def test_successful_run_with_output():
    ...
    # ensure we did not add the 'Successful run' synthetic message
    assert not any("Successful run" in msg for msg in client.messages), f"Did not expect 'Successful run' when there is output, got: {client.messages}"