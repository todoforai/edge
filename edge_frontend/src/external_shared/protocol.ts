...
import type { AgentSettingsResponse } from './REST_types';
import type { MCPJSON, MCPToolSkeleton, InstalledMCP, AttachmentDataRaw, AttachmentFrame, CallToolResult } from '../edge/types';
...
export interface TodoGracefulStopMessage {
  type: Front2Agent.TODO_GRACEFUL_STOP;
  payload: {
    projectId: string;
    todoId: string;
    userMessageId?: string;
...
export interface MessageUpdateMessage {
  ...
  payload: {
    ...
    updates: {
      content?: string;
      attachments?: AttachmentDataRaw[]; // TODO this is legacy, to no breaking change
      $append?: {
        runMeta?: RunMeta[];
        attachments?: AttachmentDataRaw[];
        [key: string]: any;
      };
      [key: string]: any;
    };
...
export interface TaskNewMessage {
  ...
  payload: {
    ...
    projectId: string;
    content: string;
    agentSettingsData: AgentSettingsResponse;
